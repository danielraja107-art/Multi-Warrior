...
async function main() {
  ...
  await until(
    ...
    'boss health 300 consumed',
  );
  check(fresh().gameUI.bossHealth === 300, `boss: health is 300`);
  check(fresh().gameUI.bossMaxHealth === 500, `boss: maxHealth is 500`);
  check(fresh().gameUI.bossPhase === BossPhase.PHASE_2, 'boss: phase is PHASE_2');
